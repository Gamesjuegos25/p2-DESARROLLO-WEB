...
import type { Vacancy, Department, estadoStatus } from '../types';
import VacancyCard from '../components/vacantesCard';
import StatsBadge from '../components/StatsBadge';
import FormField from '../components/FormField';
import Modal from '../components/Modal';
import EmployeeForm from '../components/vacantesForm';
import { useVacancy, useCreateVacancy, useUpdateVacancy, useDeleteVacancy } from '../hooks/useVacancy';
...
const nextStatus: Record<estadoStatus, estadoStatus> = {
  abierta: 'cerrada',
  cerrada: 'abierta',
};
...
function VacancyPage() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<estadoStatus | ''>('');

  const { data, isLoading: loading, isError, error: queryError } = useVacancy({
  ...
  const { data: allData } = useVacancy({});
  const allVacancies = useMemo(() => allData?.data ?? [], [allData]);
  const totalVacancies = allVacancies.length;
  const openVacancies = allVacancies.filter(vacancy => vacancy.estado === 'abierta').length;
  const closedVacancies = allVacancies.filter(vacancy => vacancy.estado === 'cerrada').length;
  ...
  const handleSelectVacancy = useCallback((vacancy: Vacancy) => {
    alert(`Vacante: ${vacancy.puesto}\nDepartamento: ${vacancy.departamento}\nModalidad: ${vacancy.modalidad}`);
  }, []);

  const handleDeleteVacancy = useCallback((id: number) => {
    if (!confirm('¿Estás seguro de eliminar esta vacante?')) return;
    deleteVacancy.mutate(id);
  }, [deleteVacancy]);

  const handleToggleStatus = useCallback((vacancy: Vacancy) => {
    updateVacancy.mutate({ id: vacancy.id, data: { estado: nextStatus[vacancy.estado] } });
  }, [updateVacancy]);
  ...
  const statuses: estadoStatus[] = ['abierta', 'cerrada'];
  const statusLabels: Record<estadoStatus, string> = {
    abierta: 'Abierta',
    cerrada: 'Cerrada',
  };
      ...
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {loading ? 'Cargando...' : `${vacancies.length} de ${totalVacancies} vacantes`}
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 flex flex-wrap items-end gap-3">
        ...
        <FormField label="Estado" className="min-w-[160px]">
          <select
            ...
            onChange={(e) => setSelectedStatus(e.target.value as estadoStatus | '')}
            className={formFieldClass}
          >
